...
(function(){
  if(window.MCCOY_CLOSEST_LEAD_AUTOFILL)return;
  window.MCCOY_CLOSEST_LEAD_AUTOFILL=true;
  ...
  function selectedAddressAlreadyPresent(){
  ...
  function applyLead(lead){
    ...
    if(selectedAddressAlreadyPresent())return false;
    const input=addressInput();
    if(input){
  ...
  async function run({force=false,allowGpsPrompt=false}={}){
    if(state.busy||!window.sb?.rpc||!window.MCCOY_ACCESS?.access?.active)return;
    if(selectedAddressAlreadyPresent())return;
    const now=Date.now();
    ...
    try{
      ...
      if(data?.address)applyLead(data);
        ...
        if(display)display.textContent='No mapped McCoy lead was found near the current location.';
      }
    }catch(error){
      console.error('Closest McCoy lead auto-fill failed',error);
    }finally{state.busy=false;}
  }

  function schedule(options={}){
    clearTimeout(state.timer);
    state.timer=setTimeout(()=>run(options),120);
  }
  ...
  for(const name of ['mccoy-access-ready','mccoy-sales-hub-layout-ready','mccoy-leads-updated','mccoy-lead-pool-changed','mccoy-location-updated','mccoy-live-location-updated']){
    window.addEventListener(name,()=>schedule({force:name!=='mccoy-location-updated',allowGpsPrompt:false}));
  }
