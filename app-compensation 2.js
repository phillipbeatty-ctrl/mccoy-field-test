...
(function(){
  ...
  .rep-rankings-card{margin-bottom:12px}.rep-rankings-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;flex-wrap:wrap}.rep-rankings-head h2{margin:0;display:flex;align-items:center;gap:10px;flex-wrap:wrap}.rep-rankings-head p{margin:5px 0 0;color:#6b7280;font-size:13px}.ranking-details-toggle{font-size:11px;font-weight:800;color:#1455d9;background:#eaf0ff;border:0;padding:5px 10px;border-radius:999px;cursor:pointer}.ranking-details-panel{margin:6px 0 8px;padding:10px 12px;background:#f7f8fa;border-radius:10px}.ranking-details-panel p{margin:0;color:#6b7280;font-size:12px;line-height:1.5}.rep-ranking-controls{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.rep-ranking-controls select{border:1px solid #d1d5db;border-radius:8px;background:#fff;padding:9px 11px;color:#111827}.rep-ranking-summary{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:8px;margin:10px 0}.rep-ranking-stat{border:1px solid #e5e7eb;border-radius:12px;padding:9px;background:#fafbfc}.rep-ranking-stat span{display:block;color:#6b7280;font-size:12px}.rep-ranking-stat strong{display:block;margin-top:5px;font-size:23px;color:#111827}.rep-ranking-stat:first-child{background:#111827;border-color:#111827}.rep-ranking-stat:first-child span,.rep-ranking-stat:first-child strong{color:#fff}.rep-records-title{margin:2px 0 7px;font-size:14px}.rep-record-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-bottom:10px}.rep-record-stat{border:1px solid #dbeafe;border-radius:12px;padding:9px;background:#f8fbff}.rep-record-stat span{display:block;color:#4b5563;font-size:11px}.rep-record-stat strong{display:block;margin-top:4px;font-size:21px;color:#1d4ed8}.rep-record-stat small,.rep-ranking-record small{display:block;margin-top:3px;color:#6b7280;font-size:10px;white-space:nowrap}.rep-ranking-record strong{display:block;color:#1d4ed8}.rep-ranking-person{font-size:12px;color:#6b7280;margin:0 0 8px}.rep-ranking-table-wrap,.pay-scale-wrap{overflow-x:auto}.rep-ranking-table,.pay-scale-table{width:100%;border-collapse:collapse;min-width:1100px}.rep-ranking-table th,.rep-ranking-table td,.pay-scale-table th,.pay-scale-table td{text-align:left;padding:7px 8px;border-bottom:1px solid #eef0f2;font-size:12px}.rep-ranking-table th,.pay-scale-table th{color:#6b7280;font-weight:600}.rep-ranking-table tbody tr.current-rep{background:#eff6ff}.rep-ranking-table tbody tr.current-rep td:first-child{font-weight:700;color:#1d4ed8}.rep-ranking-you{display:inline-block;margin-left:7px;padding:2px 7px;border-radius:999px;background:#dbeafe;color:#1d4ed8;font-size:10px;font-weight:600}.rep-ranking-empty{padding:15px 0;color:#6b7280;font-size:13px}@media(max-width:1100px){.rep-ranking-summary{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:900px){.rep-record-grid{grid-template-columns:1fr 1fr}}@media(max-width:720px){.rep-ranking-summary{grid-template-columns:1fr 1fr}.rep-ranking-stat:first-child{grid-column:1/-1}.rep-rankings-head{align-items:stretch}.rep-ranking-controls{width:100%}.rep-ranking-controls select{flex:1}}
  ...
  if(field){
    ...
    const recruiting=document.createElement('div');recruiting.className='pay-progress-card';recruiting.id='recruitingOverrideCard';recruiting.hidden=true;recruiting.innerHTML='<strong>Recruiting Override</strong><div id="recruitingOverrideMain" class="pay-progress-main"></div><div id="recruitingOverrideSub" class="pay-progress-sub"></div>';pay.insertAdjacentElement('afterend',recruiting);
    const trainerOverride=document.createElement('div');trainerOverride.className='pay-progress-card';trainerOverride.id='trainerOverrideCard';trainerOverride.hidden=true;trainerOverride.innerHTML='<strong>Trainer Override</strong><div id="trainerOverrideMain" class="pay-progress-main"></div><div id="trainerOverrideSub" class="pay-progress-sub"></div>';recruiting.insertAdjacentElement('afterend',trainerOverride);
  ...
  function renderGhostAdminSettings(data){
    ...
    // The real, current goal values live on the Ghost ranking entry itself
    // (today_sales/week_sales/month_sales/year_sales, populated from these
    // same admin-set goals server-side) -- ghost_admin_settings never
    // contained day_goal/week_goal/month_goal/year_goal, or a "minimums"
    // object, at all. Reading from the wrong object meant every field
    // always silently fell back to a hardcoded floor unless the admin had
    // personally just typed into that specific box, and Save would then
    // resubmit that wrong fallback for every untouched field, overwriting
    // genuine data with old floor numbers in whatever combination happened
    // to be untouched on a given save.
    const ghostEntry=(Array.isArray(data.rankings)?data.rankings:[]).find(rep=>rep?.is_ghost)||{};
    const currentValues={day_goal:ghostEntry.today_sales,week_goal:ghostEntry.week_sales,month_goal:ghostEntry.month_sales,year_goal:ghostEntry.year_sales};
    const fields=[
      ['ghostDayGoal','day_goal',0],
      ['ghostWeekGoal','week_goal',0],
      ['ghostMonthGoal','month_goal',0],
      ['ghostYearGoal','year_goal',0]
    ];
    ...
    for(const [id,key,minimum] of fields){
      const input=document.getElementById(id),value=String(Number(currentValues[key]??minimum));
  ...
  function ensureDashboardRankings(){
    ...
    card.querySelector('#rankingDetailsToggle').onclick=function(){
      const panel=document.getElementById('rankingDetailsPanel');
      const open=panel.hidden;
      panel.hidden=!open;
      this.setAttribute('aria-expanded',String(open));
      this.textContent=open?'How rankings work ▴':'How rankings work ▾';
    };
  ...
  async function loadRecruitingOverride(){try{const {data:d,error}=await sb.rpc('my_recruiting_override_progress');if(error||!d?.ok)return;const card=document.getElementById('recruitingOverrideCard');if(!card)return;const reps=d.recruited_reps||[];if(!reps.length){card.hidden=true;return;}card.hidden=false;const main=document.getElementById('recruitingOverrideMain'),sub=document.getElementById('recruitingOverrideSub');const total=Number(d.total_recruiting_override||0).toLocaleString('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0});window.MCCOY_UI.text(main,`${total} across ${reps.length} recruited rep${reps.length===1?'':'s'}`);const detail=reps.map(r=>`${r.rep_display_name}: ${r.overridable_sales}/${r.threshold} sales${r.threshold_reached?' (program complete)':''}`).join(' · ');window.MCCOY_UI.text(sub,`$${d.amount_per_sale}/qualifying sale until each rep's program threshold. ${detail}`);}catch(e){console.error('Recruiting override progress failed',e);}}
  async function loadTrainerOverride(){try{const {data:d,error}=await sb.rpc('my_trainer_override_progress');if(error||!d?.ok)return;const card=document.getElementById('trainerOverrideCard');if(!card)return;const trainees=d.trainees||[];if(!trainees.length){card.hidden=true;return;}card.hidden=false;const main=document.getElementById('trainerOverrideMain'),sub=document.getElementById('trainerOverrideSub');const total=Number(d.total_trainer_override||0).toLocaleString('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0});window.MCCOY_UI.text(main,`${total} across ${trainees.length} trainee${trainees.length===1?'':'s'}`);const detail=trainees.map(t=>`${t.trainee_display_name}: ${t.qualifying_sales} sale${t.qualifying_sales===1?'':'s'}`).join(' · ');window.MCCOY_UI.text(sub,`$${d.amount_per_sale}/qualifying sale during each trainee's first two Commission Weeks. ${detail}`);}catch(e){console.error('Trainer override progress failed',e);}}
  ...
  loadRecruitingOverride=window.MCCOY_UI.coalesceRefresh(loadRecruitingOverride);
  loadTrainerOverride=window.MCCOY_UI.coalesceRefresh(loadTrainerOverride);
  const poll=setInterval(()=>{if(!window.MCCOY_ACCESS?.user)return;clearInterval(poll);loadPayProgress();loadLeaders();loadRecruitingOverride();loadTrainerOverride();probe();},400);
  window.MCCOY_UI.backgroundRefresh(()=>{loadPayProgress();loadLeaders();loadRecruitingOverride();loadTrainerOverride();});
  window.MCCOY_REFRESH_RANKINGS=()=>Promise.all([loadPayProgress(),loadLeaders()]);
  window.addEventListener('mccoy-sale-saved',()=>{loadPayProgress();loadLeaders();loadRecruitingOverride();loadTrainerOverride();});
