...
(function(){
  ...
  async function loadPayProgress(){try{const d=await invoke('pay-progress');if(!d?.ok)return;const main=document.getElementById('payProgressMain'),sub=document.getElementById('payProgressSub');if(!main||!sub)return;const estimate=Number(d.estimated_weekly_commission||0).toLocaleString('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0});if(!d.pay_level_assigned){main.textContent=`${d.weekly_sales} sales this week — pay level not assigned`;sub.textContent='Ask an Admin to assign your commission pay level before the next sale. Verified sales remain recorded, but an exact commission estimate is unavailable.';return;}const progress=d.next_threshold==null?'highest extra-pay tier reached':`${d.sales_needed_for_next} more to reach ${d.next_threshold}`;main.textContent=`${d.pay_level_label} · ${d.weekly_sales} sales · ${progress}`;sub.textContent=`Estimated qualifying commission: ${estimate}. Current production increase: +$${d.current_increase_per_sale}/sale.${d.unpriced_sales?` ${d.unpriced_sales} sale${d.unpriced_sales===1?'':'s'} need Accounting review.`:''} ISP verification, Admin approval when required, installs, provider payment, and chargebacks control final pay.`;}catch(e){console.error('Pay progress failed',e);}}
