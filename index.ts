  ...
  try{
    ...
    const stats=async()=>{
      const [total,mapped,googleVerified,preserved,review,pending]=await Promise.all([
        ...
        count(q=>q.in('geocode_verification_status',['google_low_precision','google_address_mismatch','google_no_match','google_invalid_location','google_api_error','address_validation_admin_review','google_gross_relocation_flagged'])),
    ...
    const centerLat=Number.isFinite(Number(body.center_lat))?Number(body.center_lat):null
    const centerLng=Number.isFinite(Number(body.center_lng))?Number(body.center_lng):null
    const {data:rows,error:claimError}=await admin.rpc('claim_leads_for_google_verification',{
      ...
      p_center_lat:centerLat,p_center_lng:centerLng
    })
