...
(function(){
  ...
  panel.innerHTML='<div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><strong>DISTANCE TO LEAD</strong><span id="distanceLeadMode" style="font-size:11px;font-weight:900;padding:3px 7px;border-radius:999px;background:#dbeafe">AUTO</span></div>'
    ...
    +'<div id="distanceLeadRule" class="muted small">Non-sale door outcomes require a fresh GPS fix within ¼ mile. Completed sales are allowed at any distance.</div>';
  ...
  function calculate(){
    ...
    if(!manualLeadLocked&&!state.activeDoorVisit){
  ...
  function render(){
    ...
    if(mode){mode.textContent=manualLeadLocked?'MANUAL':'AUTO';mode.style.background=manualLeadLocked?'#fef3c7':'#dbeafe';}
    if(correct)correct.hidden=!state.activeDoorVisit;
    select.disabled=!!state.activeDoorVisit;
    if(manualWrap)manualWrap.hidden=!!current.withinRange;
  ...
  async function correctLead(){
    ...
    const visitStatus=byId('doorVisitStatus');if(visitStatus)visitStatus.textContent='Previous auto-selection cancelled. Choose the correct assigned lead.';select.focus();render();
  ...
  function saleContext(){
    const current=render(),manual=byId('outsideSaleAddress')?.value||'',leadAddress=label(current.lead);
  ...
  byId('useClosestLeadBtn').addEventListener('click',()=>{manualLeadLocked=false;const nearest=core.nearestLead(state.leads||[],state.latestGps||null);if(nearest&&nearest.distance<=core.QUARTER_MILE_METERS)chooseLead(nearest.lead,true);render();});
