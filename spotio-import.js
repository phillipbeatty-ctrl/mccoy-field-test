...
function resultSummary(result) {
  return [
    ...
    `${result.missing_retained || 0} missing but retained`,
    `${result.archived || 0} explicitly archived`,
    `${result.collisions || 0} collisions`,
...
uploadBtn.addEventListener('click', async () => {
  try {
    ...
    const init = await callImport(session, {
      action: 'init',
    ...
    uploadMsg.textContent = type === 'csv'
      ? 'Normalizing CSV leads into the live Lead Pool...'
      : 'Classifying and normalizing SPOTIO leads...'

    const result = await callImport(session, { action: 'normalize', batch_id: batchId })
    const summary = resultSummary(result)
    if (result.complete) {
      uploadMsg.style.color = '#166534'
      uploadMsg.textContent = `Import COMPLETE. ${summary}. Live pool: ${result.active_leads_retained || 0}. Batch ${batchId}.`
    } else {
      uploadMsg.style.color = '#991b1b'
      uploadMsg.textContent = `Import NEEDS REVIEW. ${summary}. ${result.capture_missing || 0} missing from capture. No omitted live leads were removed. Batch ${batchId}.`
    }
  } catch (error) {
    uploadMsg.style.color = '#991b1b'
