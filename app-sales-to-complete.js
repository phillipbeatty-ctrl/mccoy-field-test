...
(function(){
  ...
  function render(){
    ...
    root.innerHTML=rows.map((s,index)=>{const missing=Array.isArray(s.required_metrics_missing)?s.required_metrics_missing:[],isOpen=openId===s.id,status=s.required_metrics_complete===true?'<span class="stc-badge stc-complete">READY FOR ISP / ADMIN REVIEW</span>':'<span class="stc-badge">DETAILS PENDING</span>';return `<article class="stc-row"><div class="stc-head"><div><strong>${esc(s.isp||'Provider')} · ${esc(s.service_address||s.lead_label||'Address not entered')}</strong><div class="muted small">${new Date(s.created_at).toLocaleString()} · ${status}${missing.length?` · Missing: ${esc(missing.join(', '))}`:''}</div></div><button class="assign-btn" data-stc-open="${esc(s.id)}">${isOpen?'Close':'ENTER / EDIT CUSTOMER INFO'}</button></div>${isOpen?`<div class="stc-form">
      <div class="stc-photo"><strong>Attach ISP order photo</strong><div class="muted small">Photo extraction only fills draft suggestions. You must confirm them, and Admin must still approve the sale.</div><div class="stc-photo-options"><label class="stc-photo-option">Take a photo<input id="stcPhoto${index}" type="file" accept="image/jpeg,image/png,image/webp" capture="environment"></label><label class="stc-photo-option">Upload from your photos<input id="stcPhotoGallery${index}" type="file" accept="image/jpeg,image/png,image/webp"></label></div><div id="stcPhotoStatus${index}" class="stc-photo-status">No order photo selected.</div></div>
      ${field(`stcFirst${index}`,'Customer first name',s.customer_first_name)}${field(`stcLast${index}`,'Customer last name',s.customer_last_name)}${field(`stcPhone${index}`,'Phone',s.customer_phone)}${field(`stcEmail${index}`,'Email',s.customer_email,'email')}${field(`stcAddress${index}`,'Service address',s.service_address,'text',true)}${field(`stcOrder${index}`,'Provider order number',s.provider_order_number)}${field(`stcAccount${index}`,'Provider account number',s.provider_account_number)}${field(`stcOrderDate${index}`,'Order date',s.order_date,'date')}${field(`stcInstall${index}`,'Install date',s.install_date,'date')}${field(`stcIsp${index}`,'Internet provider',s.isp)}${field(`stcProduct${index}`,'Internet product',s.internet_product)}${field(`stcSpeed${index}`,'Internet speed Mbps',s.internet_speed_mbps,'number')}<label>Account type<select id="stcAccountType${index}"><option value="residential" ${(s.account_type||'residential')==='residential'?'selected':''}>Residential</option><option value="business" ${s.account_type==='business'?'selected':''}>Business</option></select></label><label class="stc-wide" style="flex-direction:row;align-items:center;display:flex;gap:8px"><input id="stcMigration${index}" type="checkbox" style="width:auto" ${s.is_migration?'checked':''}> Copper-to-fiber migration (not a new subscriber)</label><label class="stc-wide">Notes<textarea id="stcNotes${index}" rows="3">${esc(s.notes||'')}</textarea></label><div class="stc-wide" style="display:flex;justify-content:flex-end"><button class="stc-save" data-stc-save="${esc(s.id)}" data-index="${index}">SAVE CUSTOMER INFO</button></div></div>`:''}</article>`;}).join('');
    ...
    rows.forEach((s,index)=>{
      const cameraInput=byId(`stcPhoto${index}`);if(cameraInput)cameraInput.onchange=()=>{const file=cameraInput.files?.[0];if(file)uploadAndExtract(s.id,index,file);};
      const galleryInput=byId(`stcPhotoGallery${index}`);if(galleryInput)galleryInput.onchange=()=>{const file=galleryInput.files?.[0];if(file)uploadAndExtract(s.id,index,file);};
    });
  }
  ...
  async function save(id,index,button){const sale=rows.find(row=>row.id===id);if(!sale)return;button.disabled=true;button.textContent='SAVING…';const corrections={customer_first_name:value(`stcFirst${index}`),customer_last_name:value(`stcLast${index}`),customer_phone:value(`stcPhone${index}`),customer_email:value(`stcEmail${index}`),service_address:value(`stcAddress${index}`),provider_order_number:value(`stcOrder${index}`),provider_account_number:value(`stcAccount${index}`),order_date:value(`stcOrderDate${index}`),install_date:value(`stcInstall${index}`),isp:value(`stcIsp${index}`),internet_product:value(`stcProduct${index}`),internet_speed_mbps:value(`stcSpeed${index}`),notes:value(`stcNotes${index}`),account_type:value(`stcAccountType${index}`)||'residential',is_migration:byId(`stcMigration${index}`)?.checked||false};try{const {data,error}=await sb.rpc('save_my_sale_details',{p_sale_id:id,p_corrections:corrections});if(error)throw error;const photoId=pendingPhotoBySale[id];if(photoId)await sb.functions.invoke('sale-order-photo',{body:{action:'confirm',photo_id:photoId}}).catch(()=>{});byId('stcMessage').textContent='Customer information saved and rep-confirmed. The sale remains pending until ISP/Admin verification.';rows=rows.map(row=>row.id===id?data:row);render();window.dispatchEvent(new CustomEvent('mccoy-sale-details-updated',{detail:{saleId:id}}));}catch(error){byId('stcMessage').textContent=error?.message||'Customer information could not be saved.';button.disabled=false;button.textContent='SAVE CUSTOMER INFO';}}
  ...
  // Immediate confirmation now happens in a dedicated popup (see
  // app-sale-order-photo-confirm-popup.js), triggered by the same
  // mccoy-sale-order-photo-extracted event. This card stays a passive,
  // browsable fallback -- refreshing its list on that event so a
  // just-extracted sale appears here too if the rep reviews later -- but no
  // longer force-switches the view or auto-scrolls, since the popup already
  // owns that immediate moment and doing both would be redundant.
  window.addEventListener('mccoy-sale-order-photo-extracted',()=>{rows=[];load(true);});
})();
