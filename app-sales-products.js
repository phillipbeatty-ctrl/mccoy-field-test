// McCoy sale product options: VoIP for ISPs and expanded AT&T product capture.
(function(){
  ...
  function ensureProductUi(){
    ...
    if(!byId('attProductOptions')){
      const box=document.createElement('div');box.id='attProductOptions';box.className='sale-product-box';
      box.innerHTML=`<strong>AT&T products on this order</strong><div class="sale-product-options"><label>Mobile lines <input id="saleAttMobileLinesMirror" type="number" min="0" max="20" value="0"></label><label>Devices <input id="saleAttDeviceCount" type="number" min="0" max="20" value="0"></label><label><input id="saleAttDeviceProtection" type="checkbox"> Device protection</label><label><input id="saleAttTotalHomeCare" type="checkbox"> Total Home Care</label></div><div class="sale-product-hint">Choose Fiber or AT&T Air above. VoIP home phone lines can be recorded with the internet order.</div>`;
      grid.appendChild(box);
      const mirror=byId('saleAttMobileLinesMirror'),original=byId('saleMobile');
  ...
  function syncProviderUi(){
    ...
    const att=byId('attProductOptions');
    if(voip){voip.style.display=isIsp?'block':'none';if(!isIsp)voip.value='0';}
    if(att)att.style.display=isAtt?'block':'none';
    if(isAtt){
    ...
    if(!isAtt){
      const original=byId('saleMobile');if(original)original.value='0';
      const mirror=byId('saleAttMobileLinesMirror');if(mirror)mirror.value='0';
      const devices=byId('saleAttDeviceCount');if(devices)devices.value='0';
  ...
  function readExtras(){
    const isAtt=byId('sessionIsp')?.value==='AT&T';
    return {
      voip_home_phone_lines:Math.max(0,Math.min(20,Math.round(Number(byId('saleVoipLines')?.value||0)))),
      att_device_count:isAtt?Math.max(0,Math.min(20,Math.round(Number(byId('saleAttDeviceCount')?.value||0)))):0,
      att_device_protection:isAtt&&!!byId('saleAttDeviceProtection')?.checked,
      att_total_home_care:isAtt&&!!byId('saleAttTotalHomeCare')?.checked
    };
  }
