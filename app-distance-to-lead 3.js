...
(function(){
  ...
  async function fetchRealWorldNearest(gps){
    ...
    const {data,error}=await window.sb.functions.invoke('reverse-geocode-nearest-address',{body:{lat:gps.lat,lng:gps.lng,accuracy:gps.accuracy}});
  ...
  function maybeFallbackToRealWorld(gps,localNearest){
    if(!gps||!window.sb?.functions)return;
