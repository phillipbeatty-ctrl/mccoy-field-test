// V9.2 secure session initializer.
...
(function(){
  ...
  startBtn.addEventListener('click',async(e)=>{
    e.preventDefault();e.stopImmediatePropagation();if(state.session)return;setStartBusy(true,'VERIFYING…');telemetryStatus('Verifying secure field session…',true);
    try{
      ...
      const access=window.MCCOY_ACCESS?.access;if(!access?.role)throw new Error('Your McCoy Field access could not be verified. Sign out and sign back in.');
      const [minimumVersion,noticeVersion]=await Promise.all([oneConfig('min_supported_version'),oneConfig('privacy_notice_version')]);
      if(minimumVersion!==CLIENT_VERSION){document.getElementById('updateGate')?.classList.add('show');throw new Error(`Update required. Server requires ${minimumVersion||'a newer version'}.`);}
      ...
      document.getElementById('geoBox').textContent=gps?`Start GPS: ${gps.lat.toFixed(6)}, ${gps.lng.toFixed(6)} (±${Math.round(gps.accuracy)}m)`:'Session connected; waiting for a location fix.';updateGpsQualityBox(gps?{...gps,ageMs:0}:null);telemetryStatus('Live test telemetry connected.',true);setStartBusy(false);
    ...
    }catch(err){console.error('Secure session start failed',err);telemetrySessionId=null;fail(err?.message||'Secure session could not be started.');}
  },true);
})();