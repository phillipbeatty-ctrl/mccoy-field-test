...
test('Door Workflow no longer auto-selects from a local lead cache',()=>{
  // Address auto-population is fully retired from this file -- it belongs
  // solely to app-closest-lead-autofill-v2.js's real-world lookup now.
  // core.nearestLead is still legitimately called once, for departure
  // detection on an already-active visit -- that's a separate, correct,
  // unchanged concern, not address auto-population.
  assert.doesNotMatch(distance,/chooseLead\(nearest\.lead,true\)/)
  assert.doesNotMatch(distance,/function useClosest/)
  assert.match(distance,/This function now only reports distance\/arrival/)
