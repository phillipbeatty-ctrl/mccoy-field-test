...
(function(){
  ...
  css.textContent=`#userAdminBtn{position:fixed;right:14px;bottom:58px;z-index:2600;display:none;border:0;border-radius:999px;padding:9px 13px;background:#374151;color:#fff;font-size:12px;cursor:pointer}#userAdminPanel{position:fixed;inset:0;z-index:140001;background:rgba(17,24,39,.78);display:none;align-items:center;justify-content:center;padding:16px}#userAdminPanel.show{display:flex}.user-admin-card{width:min(1040px,100%);max-height:92vh;overflow:auto;background:#fff;border-radius:16px;padding:20px}.user-admin-row{padding:12px 0;border-top:1px solid #eef0f2}.user-admin-grid{display:grid;grid-template-columns:1fr 1fr 1.15fr 1.15fr auto;gap:8px;align-items:center}.user-password-actions{margin-top:10px}.user-password-form{display:grid;grid-template-columns:1fr 1fr auto auto;gap:8px;align-items:center;margin-top:8px}.user-password-form[hidden]{display:none!important}.user-password-form input{min-width:0;padding:9px;border:1px solid #d1d5db;border-radius:8px}.user-password-message{margin-top:6px;font-size:12px}@media(max-width:700px){.user-password-form{grid-template-columns:1fr}.user-admin-grid{grid-template-columns:1fr}}`;
  ...
  async function loadUsers(){
    ...
    try{
      const d=await onboarding('list_users');const users=d?.users||[],currentAdminEmail=String(window.MCCOY_ACCESS?.user?.email||'').toLowerCase(),admins=users.filter(u=>u.role==='admin'&&u.active),managers=users.filter(u=>u.active&&['manager','admin'].includes(u.role));
