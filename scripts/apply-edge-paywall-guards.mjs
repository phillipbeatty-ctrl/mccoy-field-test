...
await applyDeterministicTransform('rep-onboarding','mixed_pre_membership_admin',source=>{
  ...
  const replacements=[
    ...
    ["admin.from('app_user_access').select('email,display_name,role,active,assigned_manager_email,assigned_admin_email').eq('email',mgrEmail)","admin.from('app_user_access').select('email,display_name,role,active,assigned_manager_email,assigned_admin_email').eq('organization_id',callerAccess.organization_id).eq('email',mgrEmail)",'manager validation'],
    ["admin.from('app_user_access').select('email,display_name,role,active').eq('email',adminEmail)","admin.from('app_user_access').select('email,display_name,role,active').eq('organization_id',callerAccess.organization_id).eq('email',adminEmail)",'administrator validation'],
    ["admin.from('teams').select('id').eq('name',team).maybeSingle()","admin.from('teams').select('id').eq('organization_id',callerAccess.organization_id).eq('name',team).maybeSingle()",'profile team scope'],
    ["admin.from('users').upsert({id:account.id,auth_user_id:account.id,email:targetEmail.toLowerCase()","admin.from('users').upsert({id:account.id,auth_user_id:account.id,organization_id:callerAccess.organization_id,email:targetEmail.toLowerCase()",'profile organization scope'],
