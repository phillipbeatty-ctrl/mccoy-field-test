...
async function configureAndroid(){
  ...
  const permissions=['android.permission.CAMERA','android.permission.ACCESS_COARSE_LOCATION','android.permission.ACCESS_FINE_LOCATION'];
  ...
  if(!manifest.includes('android:usesCleartextTraffic='))manifest=manifest.replace(/<application\b/,'<application android:usesCleartextTraffic="false"');
  const deepLink=`\n            <intent-filter>\n                <action android:name="android.intent.action.VIEW" />\n                <category android:name="android.intent.category.DEFAULT" />\n                <category android:name="android.intent.category.BROWSABLE" />\n                <data android:scheme="fieldcoach" />\n                <data android:scheme="mccoy" />\n            </intent-filter>\n`;
  if(!manifest.includes('android:scheme="fieldcoach"')){
  ...
  if(!/targetSdkVersion\s*=\s*36\b/.test(variables))throw new Error('Native configuration failed: Capacitor Android must target API 36.');
  console.log(`Configured Field Coach Android ${versionName} (${versionCode}) with foreground location and camera permissions.`);
}
...
async function configureIos(){
  ...
  plist=setPlistString(plist,'CFBundleDisplayName','Field Coach');
  const entries=[
  ...
  if(!plist.includes('<string>fieldcoach</string>')){
    const urlTypes='\t<key>CFBundleURLTypes</key>\n\t<array>\n\t\t<dict>\n\t\t\t<key>CFBundleURLName</key>\n\t\t\t<string>com.mccoyplatform.app</string>\n\t\t\t<key>CFBundleURLSchemes</key>\n\t\t\t<array>\n\t\t\t\t<string>fieldcoach</string>\n\t\t\t\t<string>mccoy</string>\n\t\t\t</array>\n\t\t</dict>\n\t</array>\n';
    plist=insertBeforeOnce(plist,'</dict>',urlTypes,'<string>fieldcoach</string>');
  }
  if(!plist.includes('<key>UIRequiresFullScreen</key>'))plist=insertBeforeOnce(plist,'</dict>','\t<key>UIRequiresFullScreen</key>\n\t<false/>\n','<key>UIRequiresFullScreen</key>');
  ...
  console.log(`Configured Field Coach iOS ${versionName} for iPhone and iPad with explicit foreground-location, camera, and photo descriptions.`);
}
