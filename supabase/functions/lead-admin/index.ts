...
serveWithOrganizationAccess('lead_management', async request => {
  ...
  try {
    ...
    if (action === 'list_real_leads') {
      ...
      const columns = [
        'id','source_id','provider','provider_lead_id','canonical_identity_key','fallback_identity_key',
        'address1','address2','city','state','zip','latitude','longitude','pin_location_updated_at',
    ...
    if (action === 'move_lead_pin') {
      ...
      const proposedLatitude = Number(body.proposed_latitude)
      const proposedLongitude = Number(body.proposed_longitude)
      if (!leadId) return json({ error: 'lead_id_required' }, 400)
      if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(requestId)) {
        return json({ error: 'valid_client_request_id_required' }, 400)
      }
      if (!Number.isFinite(proposedLatitude) || !Number.isFinite(proposedLongitude)
          || proposedLatitude < -90 || proposedLatitude > 90
          || proposedLongitude < -180 || proposedLongitude > 180) {
        return json({ error: 'invalid_coordinates' }, 400)
      }
      const nullableNumber = (value: unknown) =>
        value === null || value === undefined || value === '' ? null : Number(value)
      const originalLatitude = nullableNumber(body.original_latitude)
      const originalLongitude = nullableNumber(body.original_longitude)
      const actorLatitude = nullableNumber(body.actor_latitude)
      const actorLongitude = nullableNumber(body.actor_longitude)
      const actorAccuracy = nullableNumber(body.actor_accuracy_meters)
      if ([originalLatitude, originalLongitude, actorLatitude, actorLongitude, actorAccuracy]
        .some(value => value !== null && !Number.isFinite(value))) {
        return json({ error: 'invalid_numeric_input' }, 400)
      }
      const { data, error } = await db.rpc('move_lead_pin', {
        p_lead_id: leadId,
        p_actor_user_id: profile.id,
        p_actor_email: email,
        p_expected_updated_at: body.expected_updated_at || null,
        p_original_latitude: originalLatitude,
        p_original_longitude: originalLongitude,
        p_proposed_latitude: proposedLatitude,
        p_proposed_longitude: proposedLongitude,
        p_actor_latitude: actorLatitude,
        p_actor_longitude: actorLongitude,
        p_actor_accuracy_meters: actorAccuracy,
        p_gps_captured_at: body.gps_captured_at || null,
        p_client_request_id: requestId,
        p_client_context: {
          platform: String(body?.client_context?.platform || '').slice(0, 40),
          app_version: String(body?.client_context?.app_version || '').slice(0, 40),
        },
      })
      if (error) throw error
      const status = data?.error === 'unauthorized_lead' ? 403
        : data?.error === 'stale_lead' ? 409
        : data?.error === 'lead_not_found' ? 404
        : data?.ok ? 200 : 400
