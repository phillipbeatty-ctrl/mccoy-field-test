...
Deno.serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:corsHeaders});try{
 ...
 if(action==='create_field_address'){
   ...
   if(zip&&!/^\d{5}(?:-\d{4})?$/.test(zip))return json({error:'invalid_zip_code'},400);
   const latitude=body.latitude===null||body.latitude===undefined?null:Number(body.latitude),longitude=body.longitude===null||body.longitude===undefined?null:Number(body.longitude);
