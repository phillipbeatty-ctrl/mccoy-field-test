...
import {
  ASAP_CONNECTION_STATUS,
  ASAP_PROVIDER,
  normalizeAsapBatch,
  normalizeAsapRecord,
  validateAsapFieldMap
} from './quantum-asap-adapter.mjs';
...
const sample = {
  ...
  order: { confirmation: 'ASAP-90001', submittedAt: '2026-08-19T18:30:00Z', status: 'submitted' },
...
assert.equal(ASAP_CONNECTION_STATUS, 'not_connected');
assert.equal(validateAsapFieldMap(fieldMap).ok, true);

const result = normalizeAsapRecord(sample, fieldMap);
assert.equal(result.ok, true);
assert.equal(result.row.provider, ASAP_PROVIDER);
assert.equal(result.row.order_number, 'ASAP-90001');
...
const invalid = normalizeAsapRecord({ seller: { id: 'agent-3003' } }, fieldMap);
assert.equal(invalid.ok, false);
assert.deepEqual(invalid.errors, ['order_number_or_account_number_missing']);

const batch = normalizeAsapBatch([sample, { order: { confirmation: 'ASAP-90002' } }], fieldMap);
...
console.log('Quantum ASAP adapter contract tests passed.');
