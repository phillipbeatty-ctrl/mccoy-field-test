// Brightspeed BASS payload adapter.
// This module performs no network or database operations. A transport and an
// approved field map must be supplied before a BASS connection can be enabled.

export const BASS_CONNECTION_STATUS = 'not_connected';
export const BASS_PROVIDER = 'Brightspeed';
...
export function validateBassFieldMap(fieldMap) {
  const errors = [];
...
export function normalizeBassRecord(record, fieldMap) {
  const mapCheck = validateBassFieldMap(fieldMap);
...
export function normalizeBassBatch(records, fieldMap) {
  ...
  records.forEach((record, index) => {
    const result = normalizeBassRecord(record, fieldMap);
