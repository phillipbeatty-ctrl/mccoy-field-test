...
function providerId(payload: any) {
  return pick(payload, [
    'provider_lead_id', 'strong_id', 'lead_id', 'leadId', 'dataObjectId',
    'data_object_id', 'objectId', 'object_id', 'spotio_id', 'spotioId', 'id',
  ]) || pick(payload?.pin, ['id', 'lead_id'])
}
...
function normalizedRecord(payload: any, mode: string) {
  ...
  return {
    provider_lead_id: providerId(payload) || null,
...
function recordKey(record: any, index: number) {
  ...
  const address = [record.address1, record.address2, record.city, record.state, record.zip]
    .map(compact).join('|')
  return address.replace(/\|/g, '') ? `address:${address}` : `invalid:${index}`
}
...
Deno.serve(async request => {
  ...
  try {
    ...
    if (sourceType === 'csv') {
      ...
      for (const item of items) {
        ...
        if (url.includes('/api/dataobjectssearch/list') && Array.isArray(data?.items)) {
          ...
          const id = providerId(capture) || `capture-${item.id}`
          byProvider.set(id, capture)
        }
      }
    ...
    // Deduplicate within this upload without treating omitted prior leads as deleted.
    const unique = new Map<string, any>()
    normalized.forEach((record, index) => unique.set(recordKey(record, index), record))
    const records = [...unique.values()]

    let created = 0, updated = 0, unchanged = 0
    let collisions = 0, quarantined = 0, archived = 0

    if (sourceType === 'csv') {
      const csvRows = []
      for (let index = 0; index < records.length; index++) {
        const record = records[index]
        const fallback = [record.address1, record.address2, record.city, record.state, record.zip]
          .map(compact).join('|')
    ...
    const classified = created + updated + unchanged + collisions + quarantined + archived
    const accepted = created + updated + unchanged
    const missing = expected > 0 ? Math.max(0, expected - classified) : 0
    const complete = missing === 0 && collisions === 0 && quarantined === 0
    const status = complete ? 'normalized' : 'incomplete'
    const notes = [
      `${sourceType.toUpperCase()} additive import: ${created} created, ${updated} updated,`,
      `${unchanged} unchanged, ${collisions} collisions, ${quarantined} quarantined,`,
      `${archived} explicitly archived; ${missing} missing from capture.`,
