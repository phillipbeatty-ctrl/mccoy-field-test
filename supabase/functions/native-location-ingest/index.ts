...
const denialReason=(message:string)=>String(message||'').match(/organization_access_denied:([a-z0-9_]+)/i)?.[1]||null
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const {data:profile,error:profileError}=await supabase.from('users').select('id,auth_user_id,organization_id,active').eq('id',grant.user_id).eq('organization_id',grant.organization_id).eq('active',true).maybeSingle()
    if(profileError)throw profileError
    const authUserId=profile?.auth_user_id||profile?.id
    if(!authUserId)return json({error:'active_user_profile_required'},403)

    const {error:accessError}=await supabase.rpc('service_assert_organization_access',{
      p_auth_user_id:authUserId,
      p_entitlement:'native_background_location'
    })
    if(accessError){
      const reason=denialReason(accessError.message)
      if(reason)return json({error:'organization_access_denied',reason},403)
      console.error('native-location-ingest access check failed',accessError)
