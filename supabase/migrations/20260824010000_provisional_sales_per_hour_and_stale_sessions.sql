-- Mark weekly Sales/Hr provisional until one tracked hour and automatically
-- close field sessions that have had no activity for 30 minutes. Automatic
-- closures use the final recorded event time (or a 30-minute no-event cap),
-- are capped at 16 hours, and are retained in a private audit trail.

create extension if not exists pg_cron with schema pg_catalog;

create table if not exists public.field_session_auto_closures (
  id bigint generated always as identity primary key,
  session_id uuid not null references public.test_sessions(id) on delete cascade,
  last_activity_at timestamptz not null,
  closed_at timestamptz not null,
  reason text not null default 'inactive_30_minutes'
    check (reason = 'inactive_30_minutes'),
  created_at timestamptz not null default now(),
  unique (session_id)
);

alter table public.field_session_auto_closures enable row level security;
revoke all on table public.field_session_auto_closures from public, anon, authenticated;
revoke all on sequence public.field_session_auto_closures_id_seq from public, anon, authenticated;
...
begin
  with candidates as materialized (
    ...
    where ts.ended_at is null
      ...
      and greatest(ts.started_at, coalesce(events.last_event_at, ts.started_at))
  ...
  updated as (
    ...
    returning ts.id, c.last_activity_at, ts.ended_at
  )
  insert into public.field_session_auto_closures(session_id, last_activity_at, closed_at)
...
begin
  ...
  real_objects as (
    select jsonb_build_object(
      ...
      'sales_per_hour', jsonb_build_object(
        ...
        'qualified',r.week_tracked_hours >= 1,'provisional',r.week_tracked_hours < 1,'minimum_tracked_hours',1
      ),
  ...
  select jsonb_build_object(
    ...
    'sales_per_hour_policy', 'verified_week_sales_divided_by_capped_tracked_field_session_hours_provisional_until_one_hour_stale_sessions_auto_closed_ghost_excluded',
    ...
    'leaders', jsonb_build_object(
      ...
      'sales_per_hour', (select jsonb_build_object(
        'name',rep_name,'rate',week_sales_per_hour,'tracked_hours',week_tracked_hours,'week_sales',week_sales,
        'qualified',true,'provisional',false,'minimum_tracked_hours',1
      ) from ranked where week_tracked_hours >= 1 and week_sales > 0 order by sales_per_hour_rank limit 1)
    ),
...
comment on function public.get_verified_sales_rankings() is
  'Single authoritative verified-sales ranking with faster-accumulation tie breaks and weekly verified sales/hour for every real user; rates remain provisional below one tracked hour, stale sessions auto-close, and Ghost is excluded from sales/hour and all-time records.';
