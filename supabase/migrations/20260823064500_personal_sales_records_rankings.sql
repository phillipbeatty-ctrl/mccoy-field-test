-- Extend the single authoritative ranking response with each active user's
-- best verified sales day, Monday-start week, calendar month, and calendar
-- year. Ties retain the most recent occurrence. No second ranking path is
-- created, and the existing permanent ranking-credit policy is unchanged.
...
begin
  ...
  personal_records as (
    ...
    from record_winners r
    where r.record_order=1
    group by r.rep_email
  ),
  totals as (
    select
  ...
  select jsonb_build_object(
    ...
    'personal_record_policy', 'pacific_calendar_periods_monday_week_most_recent_tie',
    ...
    'pending_review_sales', (select count(*) from public.sales_records s where s.ranking_eligible is not true and lower(coalesce(s.sale_status, '')) <> 'cancelled')
  ) into v_result
  from bounds b;
...
comment on function public.get_verified_sales_rankings() is
  'Authoritative provider-verified rankings and all-time personal day/week/month/year records for every active McCoy account; Pacific calendar periods, Monday-start weeks, and cancellations affect accounting only.';
