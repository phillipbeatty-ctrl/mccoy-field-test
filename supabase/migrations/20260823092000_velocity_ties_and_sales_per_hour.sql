-- Extend the single authoritative rankings with two ordered controls:
-- 1) equal sale totals are resolved by faster accumulation, then by the later
--    matching-sale time so a rep coming from behind takes the rank;
-- 2) weekly verified sales per tracked field-session hour are visible and
--    ranked for every real user. Ghost is excluded from this metric.
-- Open/stale sessions use last activity, active sessions use now, each session
-- is capped at 16 hours, and each user's total is capped at 16 hours per day.
...
begin
  ...
  ranked as (
    select t.*,
      ...
      case when week_tracked_hours > 0 then row_number() over (
        order by (week_tracked_hours > 0) desc, week_sales_per_hour desc nulls last,
          week_sales desc, week_elapsed_seconds asc nulls last, week_reached_at desc nulls last,
          lower(rep_name), rep_email
      )::int else null end as sales_per_hour_rank
    from totals t
  ),
  pending as (
  ...
  select jsonb_build_object(
    ...
    'sales_per_hour_policy', 'verified_week_sales_divided_by_capped_tracked_field_session_hours_ghost_excluded',
...
comment on function public.get_verified_sales_rankings() is
  'Single authoritative verified-sales ranking with faster-accumulation tie breaks, weekly verified sales/hour from tracked field sessions, and an Admin-managed Ghost benchmark; Ghost is excluded from sales/hour and all-time records.';
