...
-- Production enablement is an operator-controlled organization setting. It does
-- not enroll users individually or weaken any identity, assignment or GPS check.
create table private.field_gps_rollout (
  organization_id uuid primary key references public.organizations(id),
  enabled boolean not null default false,
  release_ref text not null,
  updated_at timestamptz not null default clock_timestamp()
);
alter table private.field_gps_rollout enable row level security;
revoke all on private.field_gps_rollout from public,anon,authenticated,service_role;
grant select on private.field_gps_rollout to service_role;
...
begin
  ...
  select enabled into v_rollout from private.field_gps_rollout where organization_id=v_access.organization_id;
  v_rollout:=coalesce(v_rollout,false);
  v_enabled:=v_rollout or coalesce(v_pilot.enabled_until>clock_timestamp(),false);
  if p_action='status' then
    return jsonb_build_object('ok',true,'enabled',v_enabled,'production_enabled',v_rollout,
      'can_manage',v_access.role='admin' and not v_rollout,'consented',v_consent,'enabled_until',case when v_rollout then null else v_pilot.enabled_until end);
  end if;
  if p_action='set_pilot' then
    ...
    if v_rollout then
      return jsonb_build_object('ok',true,'enabled',true,'production_enabled',true,'can_manage',false,'consented',v_consent);
    end if;
  ...
  if not v_enabled then raise exception 'gps_pilot_not_enabled' using errcode='42501'; end if;
...
comment on function public.field_gps_placement(uuid,text,text,jsonb) is 'Organization rollout with optional account pilot. Authenticated Edge identity only; ADD ADDRESS never moves coordinates; KNOCK DOOR places an authorized address group using an owned active manual visit. No automatic nearest-lead selection.';
commit;
