...
test('cache versions force the cleaned Sales Hub assets to replace the redundant UI',()=>{
  for(const asset of ['app-door-workflow-core.js'])assert.match(html,new RegExp(`${asset.replaceAll('.','\\.')}\\?v=\\d+`));
  assert.match(html,/app-typed-lead-address\.js\?v=2026091108/);
  for(const asset of ['app-part1.js','app-field-addresses.js','app-distance-to-lead.js','app-provider-sale-router.js'])assert.match(html,new RegExp(`${asset.replaceAll('.','\\.')}\\?v=${asset==='app-provider-sale-router.js'?'2026091105':asset==='app-distance-to-lead.js'?'2026091106':'2026091103'}`));
