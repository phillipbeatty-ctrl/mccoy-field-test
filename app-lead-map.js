(()=>{
  ...
  let correctionLead=null;
  let correctionMarker=null;

  function currentRealFiltered(){
    const team=document.getElementById('teamFilter')?.value||'';
    const q=(document.getElementById('leadSearch')?.value||'').toLowerCase();
    return (state.realLeads||[]).filter(l=>
      Number.isFinite(Number(l.lat))&&Number.isFinite(Number(l.lng))&&
      (!team||l.team===team)&&
      (!q||`${l.address} ${l.city||''} ${l.stateCode||''} ${l.zip||''} ${l.rep||''}`.toLowerCase().includes(q))
    );
  }

  function markerStyle(selected=false){
    return selected?{renderer,radius:6,weight:2,fillOpacity:.9}:{renderer,radius:4,weight:1,fillOpacity:.55};
  }

  function updateSelectionStatus(prefix=''){
    const leads=currentRealFiltered();
    const base=`${leads.length.toLocaleString()} mapped leads in current filters · ${selectedIds.size.toLocaleString()} selected.`;
    const el=document.getElementById('mapSelectionStatus');
    if(el)el.textContent=prefix?`${prefix} · ${base}`:base;
  }
  ...
  function restoreGrabCursor(){
    ...
    if(btn){btn.textContent='LASSO SELECT';btn.className='assign-btn';}
  ...
  function setMarkerSelectedStyle(leadId){
    const marker=markerByLead.get(leadId);
    if(marker)marker.setStyle(markerStyle(selectedIds.has(leadId)));
  }

  function toggleLeadSelection(l){
    if(selectedIds.has(l.dbId))selectedIds.delete(l.dbId);
    else selectedIds.add(l.dbId);
    setMarkerSelectedStyle(l.dbId);
    updateSelectionStatus(selectedIds.has(l.dbId)?'Lead added to selection':'Lead removed from selection');
  }
  ...
  function renderPins(fit=false){
    ...
    for(const l of leads){
      const m=L.circleMarker([Number(l.lat),Number(l.lng)],markerStyle(selectedIds.has(l.dbId)));
      m.bindTooltip(`${l.address}${l.rep?' · '+l.rep:''}`);
      m.on('click',e=>{
        if(lassoMode){L.DomEvent.stopPropagation(e);return;}
        toggleLeadSelection(l);
        selectCorrectionLead(l);
      });
      m.addTo(layer);
      markerByLead.set(l.dbId,m);
      bounds.push([Number(l.lat),Number(l.lng)]);
    }
    updateSelectionStatus();
    if(bounds.length&&(fit||firstFit)){
  ...
  function correctionMsg(text){
    const el=document.getElementById('leadCorrectionMsg');
    if(el)el.textContent=text;
  }

  function fillCorrectionForm(l){
    document.getElementById('editLeadAddress1').value=l.address1||l.address||'';
    document.getElementById('editLeadAddress2').value=l.address2||'';
    document.getElementById('editLeadCity').value=l.city||'';
    document.getElementById('editLeadState').value=l.stateCode||'';
    document.getElementById('editLeadZip').value=l.zip||'';
  }

  function selectCorrectionLead(l){
    if(window.MCCOY_ACCESS?.access?.role!=='admin')return;
    correctionLead=l;
    const correctionPanel=document.getElementById('leadCorrectionPanel');
    if(correctionPanel)correctionPanel.style.display='block';
    fillCorrectionForm(l);
    correctionMsg('Lead selected. Drag the large pin only if its map location needs correction.');
    ...
    correctionMarker.on('dragend',async()=>{
      const p=correctionMarker.getLatLng();
      correctionMsg('Saving corrected location…');
      try{
        const {data,error}=await sb.functions.invoke('lead-admin',{body:{action:'update_lead',lead_id:l.dbId,latitude:p.lat,longitude:p.lng}});
        if(error||!data?.ok)throw error||new Error(data?.detail||data?.error||'location_save_failed');
        l.lat=data.lead.latitude;
        l.lng=data.lead.longitude;
        markerByLead.get(l.dbId)?.setLatLng([l.lat,l.lng]);
        correctionMsg('Corrected map location saved.');
      }catch(e){
        console.error(e);
        correctionMsg('Unable to save corrected location.');
        correctionMarker.setLatLng([Number(l.lat),Number(l.lng)]);
      }finally{
        restoreGrabCursor();
      }
    });
  ...
  window.MCCOY_SELECT_MAP_LEAD=id=>{
    const l=(state.realLeads||[]).find(x=>x.id===id||x.dbId===id);
    if(l){
      if(!selectedIds.has(l.dbId))selectedIds.add(l.dbId);
      setMarkerSelectedStyle(l.dbId);
      updateSelectionStatus('Lead selected');
      selectCorrectionLead(l);
    }
  };
  ...
  async function saveAddress(){
    ...
    const btn=document.getElementById('saveLeadAddressBtn');
    btn.disabled=true;
    correctionMsg('Saving address…');
    const patch={
      action:'update_lead',
      lead_id:correctionLead.dbId,
      address1:document.getElementById('editLeadAddress1').value,
      address2:document.getElementById('editLeadAddress2').value,
      city:document.getElementById('editLeadCity').value,
      state:document.getElementById('editLeadState').value,
      zip:document.getElementById('editLeadZip').value
    };
    try{
      const {data,error}=await sb.functions.invoke('lead-admin',{body:patch});
      if(error||!data?.ok)throw error||new Error(data?.detail||data?.error||'address_save_failed');
      const r=data.lead;
      correctionLead.address1=r.address1||'';
      correctionLead.address2=r.address2||'';
      correctionLead.address=[r.address1,r.address2].filter(Boolean).join(' ');
      correctionLead.city=r.city||'';
      correctionLead.stateCode=r.state||'';
      correctionLead.zip=r.zip||'';
      correctionLead.fullAddress=[[r.address1,r.address2].filter(Boolean).join(' '),r.city,r.state,r.zip].filter(Boolean).join(', ');
      markerByLead.get(correctionLead.dbId)?.setTooltipContent(correctionLead.address);
      correctionMsg('Lead address saved.');
      if(typeof window.renderLeads==='function')window.renderLeads();
    }catch(e){
      console.error(e);
      correctionMsg('Unable to save lead address.');
    }finally{
      btn.disabled=false;
      restoreGrabCursor();
    }
  ...
  function selectVisible(){
    ...
    selectedIds=new Set(currentRealFiltered().filter(l=>b.contains([Number(l.lat),Number(l.lng)])).map(l=>l.dbId));
    restoreGrabCursor();
    renderPins(false);
    updateSelectionStatus('Current map view selected');
  }
  ...
  function beginLasso(e){
    ...
    lastLassoPoint=e.containerPoint;
    lassoStartPoint=e.containerPoint;
  ...
  async function geocodeStatus(){
    if(window.MCCOY_ACCESS?.access?.role!=='admin'){
      document.getElementById('geocodeRealLeadsBtn').style.display='none';
      document.getElementById('geocodeProgress').textContent='Lead coordinates are managed by Admin.';
      return null;
    }
    try{
      const {data,error}=await sb.functions.invoke('lead-geocode',{body:{action:'status'}});
      if(error)throw error;
      document.getElementById('geocodeProgress').textContent=`Mapped ${Number(data.geocoded||0).toLocaleString()} of ${Number(data.total||0).toLocaleString()} · ${Number(data.unmatched||0).toLocaleString()} unmatched · ${Number(data.remaining||0).toLocaleString()} not processed.`;
      return data;
    }catch(e){
      console.error(e);
      document.getElementById('geocodeProgress').textContent='Unable to read geocode status.';
      return null;
    }
  }
  ...
  async function geocodeAll(){
    ...
    try{
      let loops=0;
      while(loops++<20){
        const {data,error}=await sb.functions.invoke('lead-geocode',{body:{action:'geocode_next',limit:750}});
      ...
      setTimeout(()=>renderPins(true),250);
    }catch(e){
      console.error(e);
      document.getElementById('geocodeProgress').textContent='Geocoding stopped because of an error. You can safely press GEOCODE REAL LEADS again to resume.';
    }
    btn.disabled=false;
  ...
  async function assignSelectedLeads(){
    const msg=document.getElementById('mapAssignMsg');
    if(window.MCCOY_ACCESS?.access?.role!=='admin'){
      msg.textContent='Only Admin can assign real leads.';
      restoreGrabCursor();
      return;
    }
    if(!selectedIds.size){
      msg.textContent='Select one or more leads first.';
      restoreGrabCursor();
      return;
    }
    const repEmail=document.getElementById('mapRepSelect').value;
    const ids=[...selectedIds];
    msg.textContent=`Assigning ${ids.length.toLocaleString()} selected lead${ids.length===1?'':'s'}…`;
    try{
      for(let i=0;i<ids.length;i+=500){
        const {data,error}=await sb.functions.invoke('lead-admin',{body:{action:'assign_leads',lead_ids:ids.slice(i,i+500),rep_email:repEmail}});
        if(error||!data?.ok)throw error||new Error(data?.detail||data?.error||'bulk_assign_failed');
      }
      msg.textContent=`${ids.length.toLocaleString()} selected lead${ids.length===1?'':'s'} assigned successfully.`;
      selectedIds.clear();
      clearLassoShape();
      if(correctionMarker){map.removeLayer(correctionMarker);correctionMarker=null;}
      await window.loadMcCoyLeads?.();
      setTimeout(()=>renderPins(false),250);
    }catch(e){
      console.error(e);
      msg.textContent=`Assignment failed${e?.message?': '+e.message:''}.`;
    }finally{
      restoreGrabCursor();
    }
  }
