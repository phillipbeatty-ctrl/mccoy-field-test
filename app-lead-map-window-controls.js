(()=>{
  ...
  let mode=STANDARD,selectedLead=null,movePinLeadId=null,mountedWorkflow=null,hintTimer=null,longPressTimer=null,suppressClick=false,confirmObserver=null,statusObserver=null,statusTimer=null
  ...
  style.textContent=`
    ...
    #leadMapMoveDock{position:absolute;z-index:1410;right:max(22px,calc(env(safe-area-inset-right) + 18px));bottom:max(92px,calc(env(safe-area-inset-bottom) + 82px));display:none;align-items:center;gap:2px;padding:3px;border-radius:999px;background:rgba(255,255,255,.9);box-shadow:0 3px 14px rgba(15,23,42,.24);backdrop-filter:blur(8px)}#leadMapMoveDock.show{display:flex}#leadMapMoveDock .move-active-only{display:none!important}#leadMapMoveDock.active .move-active-only{display:grid!important}
  ...
  const maximize=byId('leadMapMaximizeBtn'),actions=byId('leadMapActionsBtn'),restore=byId('leadMapRestoreBtn'),fitAllAction=byId('leadMapFitAllAction'),dispositionAction=byId('leadMapDispositionAction'),moveAction=byId('leadMapMovePinAction'),workflowBody=byId('leadMapWorkflowBody'),moveStart=byId('leadMapMoveStart'),moveConfirm=byId('leadMapMoveConfirm'),moveCancel=byId('leadMapMoveCancel')
  ...
  function sync(){
    ...
    setAvailable(maximize,mode===STANDARD);setAvailable(actions,expanded&&!dispositionOpen&&!moveReady&&!moveActive);setAvailable(restore,expanded&&!dispositionOpen&&!moveReady&&!moveActive);setAvailable(moveStart,moveReady||moveActive);moveAction.setAttribute('aria-disabled',mayMoveSelectedLead()?'false':'true');moveAction.classList.toggle('is-disabled',!mayMoveSelectedLead());dispositionAction.setAttribute('aria-disabled',selectedLead?'false':'true');dispositionAction.classList.toggle('is-disabled',!selectedLead);syncCompactConfirm()
  ...
  function setMode(next){if(next===STANDARD)restoreMountedWorkflow();if(next!==MOVE_PIN_READY&&next!==MOVE_PIN&&movePinLeadId)releaseMovePinOwnership();mode=next;if(mode===STANDARD)status.classList.remove('show');sync()}
  ...
  handleControl(actions,'ACTIONS',()=>{if(mode===STANDARD){showHint('MAXIMIZE MAP FIRST');return}if(mode===DISPOSITION||mode===MOVE_PIN_READY||mode===MOVE_PIN){showHint('FINISH OR CANCEL CURRENT ACTION');return}setMode(mode===ACTION_MENU?EXPANDED:ACTION_MENU)})
  handleControl(restore,'RESTORE',()=>{if(mode===EXPANDED||mode===ACTION_MENU)setMode(STANDARD)})
  fitAllAction.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();window.MCCOY_LEAD_MAP?.fitLeadPins?.();mode=EXPANDED;sync();showHint('FIT ALL PINS')})
  ...
  window.addEventListener('mccoy-map-move-pin-ended',event=>{if(event.detail?.message)showMapStatus(event.detail.message);if(mode!==MOVE_PIN&&mode!==MOVE_PIN_READY)return;releaseMovePinOwnership();mode=EXPANDED;sync()})
