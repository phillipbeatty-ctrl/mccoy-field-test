(()=>{
  ...
  moveConfirm.addEventListener('click',async event=>{event.preventDefault();event.stopPropagation();syncCompactConfirm();if(moveConfirm.getAttribute('aria-disabled')==='true')return showHint('MOVE THE PIN FIRST');showMapStatus('Checking latest pin state…');const fresh=await refreshMovePinSnapshot();if(!fresh){showMapStatus('Unable to verify current pin. Try again.');return}const underlying=byId('confirmLeadPinBtn');if(typeof underlying?.onclick!=='function'){showMapStatus('Unable to start pin confirmation.');return}showMapStatus('Saving pin location…');underlying.onclick.call(underlying,event)})
