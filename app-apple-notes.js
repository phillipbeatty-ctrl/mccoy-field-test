...
(()=>{
  ...
  card.innerHTML='<div class="card-head"><div><h2>Apple Notes · Authorized Folders</h2><p class="muted">Nothing is imported unless you explicitly authorize the exact folder name.</p></div></div>'
    ...
    +'<li>On your iPhone, iPad, or Mac, open Shortcuts and create a new shortcut.</li>'
    +'<li>Add <strong>Find Notes</strong> and set its Folder filter to the exact authorized folder.</li>'
    +'<li>For each note, create a dictionary containing <code>id</code>, <code>title</code>, <code>body</code>, and optionally <code>modified_at</code>.</li>'
    +'<li>Use <strong>Get Contents of URL</strong> with method POST, a JSON body containing <code>action: sync_notes</code>, <code>folder: your exact authorized folder</code>, and <code>notes: your note dictionaries</code>.</li>'
    +'<li>Add the request header <code>x-mccoy-notes-token</code> with the generated shortcut key.</li>'
    +'<li>Run the shortcut manually, or create an Apple Shortcuts automation for that folder.</li></ol>'
  ...
  async function loadStatus(){
    try{latestStatus=await invoke('status');renderFolders(latestStatus.folders);const count=Number(latestStatus.note_count||0);setStatus((latestStatus.connection?.active?'Apple Shortcuts connection active':'Apple Shortcuts connection not configured')+' · '+count+' synced note'+(count===1?'':'s')+'.');loaded=true;}
