...
(function(){
  ...
  css.textContent=`
    ...
    .sale-bank{margin-top:18px;border-top:2px solid #e5e7eb;padding-top:14px}.sale-bank-row{padding:12px 0;border-top:1px solid #e5e7eb}.sale-bank-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;font-size:12px}.sale-bank-actions{display:grid;grid-template-columns:minmax(210px,1fr) minmax(230px,1.2fr) auto minmax(160px,.7fr) auto;gap:8px;margin-top:10px;align-items:center}.sale-bank-actions select,.sale-bank-actions input{min-width:0;padding:9px;border:1px solid #d1d5db;border-radius:8px;background:#fff}.sale-provider-fill{display:grid;grid-template-columns:minmax(300px,1.5fr) minmax(230px,1fr) auto;gap:8px;margin-top:10px;padding:10px;border:1px solid #bfdbfe;border-radius:10px;background:#eff6ff;align-items:end}.sale-provider-fill label{display:grid;gap:4px;font-size:11px;font-weight:700;color:#1e3a8a}.sale-provider-fill select,.sale-provider-fill input{min-width:0;padding:9px;border:1px solid #93c5fd;border-radius:8px;background:#fff}.sale-provider-preview{grid-column:1/-1;padding:8px;border-radius:8px;background:#fff;color:#374151;font-size:12px}.sale-incomplete-fields{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:10px}.sale-incomplete-fields label{display:grid;gap:4px;font-size:11px;font-weight:700;color:#374151}.sale-incomplete-fields input{min-width:0;padding:9px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font:inherit}.sale-incomplete-actions{display:grid;grid-template-columns:minmax(210px,1fr) minmax(230px,1.2fr) auto minmax(160px,.7fr) auto;gap:8px;margin-top:10px;align-items:center}.sale-incomplete-actions select,.sale-incomplete-actions input{min-width:0;padding:9px;border:1px solid #d1d5db;border-radius:8px;background:#fff}.sale-credit-empty{padding:18px;border:1px dashed #d1d5db;border-radius:10px;color:#6b7280}
    .sale-removed-audit{margin-top:18px;border-top:1px solid #fecaca;padding-top:14px}
    @media(max-width:980px){.sale-bank-actions,.sale-provider-fill,.sale-incomplete-actions,.sale-incomplete-fields{grid-template-columns:1fr 1fr}}@media(max-width:760px){.sale-credit-summary,.sale-credit-actions,.sale-credit-review,.sale-credit-payment,.sale-account-columns,.sale-bank-grid,.sale-bank-actions,.sale-provider-fill,.sale-incomplete-actions,.sale-incomplete-fields{grid-template-columns:1fr}}
  `;
  ...
  function ensurePanel(){
    ...
    const panel=document.createElement('div');panel.id='saleCreditPanel';panel.innerHTML=`<div class="sale-credit-card">
      ...
      <section class="sale-bank"><div class="card-head"><div><h2>Unassigned Sales Bank</h2><p class="muted small">Only active candidates appear here. NOT A SALE removes the sale and linked ISP evidence from this bank while preserving the audit record.</p></div><button id="saleBankRefresh" class="assign-btn">Refresh Bank</button></div><div id="saleBankRows">Loading…</div></section>
      <section class="sale-removed-audit"><div class="card-head"><div><h2>Removed Sales Audit</h2><p class="muted small">These records are retained only for Admin audit and restoration. They are not part of the Sales Bank, rankings, or earned-sale accounting.</p></div></div><div id="saleRemovedRows">Loading…</div></section>
  ...
  function bindSaleActions(record,index){
    ...
    reviewButton.onclick=async()=>{
      ...
      try{const {error}=await sb.rpc('admin_set_sale_review_disposition',{p_sale_id:sale.id,p_disposition:disposition,p_reason:reason});if(error)throw error;await refreshAfterChange(sale.id,disposition==='not_a_sale'?'Sale marked NOT A SALE and removed from the Sales Bank, rankings, Customer List, and earned-sale accounting.':disposition==='unverified'?'Sale marked UNVERIFIED and removed from rankings.':'Provider decision restored subject to required-data validation.');}catch(error){setNotice(error?.message||'Unable to apply Admin review.');reviewButton.disabled=false;reviewButton.textContent='Apply Review';}
    };
  ...
  async function loadBank(force=false){
    ...
    try{
      const [bankResult,candidateResult]=await Promise.all([
        sb.rpc('admin_unassigned_sales_bank'),
        sb.rpc('admin_confirmed_sale_provider_candidates')
      ]);
      if(bankResult.error)throw bankResult.error;if(candidateResult.error)throw candidateResult.error;
      state.bank=bankResult.data||{};const candidates=candidateResult.data||{};
      state.bank.incomplete_sales=(state.bank.incomplete_sales||[]).map(row=>({...row,provider_candidates:Array.isArray(candidates[row.id])?candidates[row.id]:[]}));
      renderBank();
    }catch(error){console.error(error);root.textContent=error?.message||'Unable to load the Unassigned Sales Bank.';}
  }
  ...
  function renderBank(){
    ...
    const rowMarkup=(row,index,deniedRow=false)=>`<div class="sale-bank-row"><strong>${esc(row.provider)} · ${esc(row.order_number||row.account_number||'No order/account number')}</strong>${row.admin_evidence_disposition?` · <span class="sale-credit-missing">${esc(String(row.admin_evidence_disposition).toUpperCase().replaceAll('_',' '))}</span>`:''}<div class="sale-bank-grid"><span>Seller: ${esc(row.seller_name||row.seller_email||row.seller_identifier||'Unknown')}</span><span>Customer: ${esc(row.customer_name||'Unknown')}</span><span>Status: ${esc(row.provider_status||'Unknown')}</span><span>Address: ${esc(row.service_address||'Unknown')}</span><span>Sale date: ${esc(row.sale_date||'Unknown')}</span><span>Import: ${esc(row.materialization_status||'unmaterialized')}</span></div><div class="sale-credit-evidence">${esc(row.why_considered_sale||row.admin_evidence_reason||row.materialization_reason||'Provider evidence requires review.')}</div>${deniedRow?`<div class="sale-bank-actions"><span></span><input id="saleBankReasonDenied${index}" maxlength="500" placeholder="Reason required to restore" aria-label="Provider evidence restore reason"><button id="saleBankRestoreDenied${index}" class="assign-btn">RESTORE TO SALES BANK</button><span></span><span></span></div>`:`<div class="sale-bank-actions"><select id="saleBankUser${index}" aria-label="Assign provider evidence to McCoy user">${userOptions}</select><input id="saleBankReason${index}" maxlength="500" placeholder="Assignment or review reason required" aria-label="Provider evidence decision reason"><button id="saleBankAssign${index}" class="primary">APPROVE &amp; ASSIGN</button><select id="saleBankDisposition${index}" aria-label="Provider evidence review disposition"><option value="">Choose review…</option><option value="unverified">UNVERIFIED</option><option value="not_a_sale">NOT A SALE · REMOVE FROM BANK</option>${row.admin_evidence_disposition?'<option value="restore">RESTORE</option>':''}</select><button id="saleBankReview${index}" class="assign-btn">Apply Review</button></div>`}<details class="sale-account-details"><summary>Provider evidence and review history</summary><div class="sale-account-columns"><div class="sale-account-block"><h3>ISP evidence</h3><pre class="sale-account-json">${pretty(row)}</pre></div><div class="sale-account-block"><h3>Evidence decision history</h3><pre class="sale-account-json">${pretty(row.review_history||[])}</pre></div></div></details></div>`;
    const incompleteMarkup=(row,index)=>{
      const options='<option value="">Choose active McCoy user…</option>'+state.users.map(user=>`<option value="${esc(user.email)}" ${String(user.email).toLowerCase()===String(row.rep_email).toLowerCase()?'selected':''}>${esc(user.display_name||user.email)} · ${esc(user.email)} (${esc(user.role)})</option>`).join('');
      const providerCandidates=Array.isArray(row.provider_candidates)?row.provider_candidates:[];
      const providerOptions='<option value="">Choose matching ISP export…</option>'+providerCandidates.map(candidate=>`<option value="${esc(candidate.id)}">${esc(candidate.order_number||candidate.account_number||'No order/account')} · ${esc(candidate.customer_name||'Unknown customer')} · ${esc(candidate.service_address||'No address')}${candidate.will_archive_provider_duplicate?' · MERGE DUPLICATE':''}</option>`).join('');
      return `<div class="sale-bank-row"><strong>${esc(row.provider)} · ${esc(row.order_number||'No order number')}</strong>${row.admin_review_disposition?` · <span class="sale-credit-missing">${esc(String(row.admin_review_disposition).toUpperCase().replaceAll('_',' '))}</span>`:''}<div class="sale-bank-grid"><span>Rep: ${esc(row.rep_name||row.rep_email||'Unknown')}</span><span>Customer: ${esc(row.customer_name||'Unknown')}</span><span>Address: ${esc(row.service_address||'Missing')}</span><span>Order date: ${esc(row.order_date||'Missing')}</span><span>Install date: ${esc(row.install_date||'Missing')}</span><span class="sale-credit-missing">Missing: ${esc((row.missing||[]).join(', ')||'required data')}</span></div><div class="sale-provider-fill"><label>Authoritative ISP export<select id="saleProviderCandidate${index}" aria-label="Choose ISP export for user-confirmed sale" ${providerCandidates.length?'':'disabled'}>${providerOptions}</select></label><input id="saleProviderReason${index}" maxlength="500" placeholder="Why this ISP order matches the confirmed sale" aria-label="ISP reconciliation reason" ${providerCandidates.length?'':'disabled'}><button id="saleProviderApply${index}" class="primary" disabled>FILL FROM ISP EXPORT &amp; VERIFY</button><div id="saleProviderPreview${index}" class="sale-provider-preview">${providerCandidates.length?'Choose an ISP export to preview the customer, order, address, product, and dates that can fill blank accounting fields.':'No safe ISP-export candidate is available for this user-confirmed sale.'}</div></div><div class="sale-incomplete-fields"><label>Order number<input id="saleIncompleteOrder${index}" value="${esc(row.order_number||'')}" placeholder="Required order number"></label><label>Customer first name<input id="saleIncompleteFirst${index}" value="${esc(row.customer_first_name||'')}" placeholder="Required first name"></label><label>Customer last name<input id="saleIncompleteLast${index}" value="${esc(row.customer_last_name||'')}" placeholder="Required last name"></label><label>Service address<input id="saleIncompleteAddress${index}" value="${esc(row.service_address||'')}" placeholder="Required service address"></label><label>Install date<input id="saleIncompleteInstall${index}" type="date" value="${esc(row.install_date||'')}"></label><label>Order date<input id="saleIncompleteDate${index}" type="date" value="${esc(row.order_date||'')}"></label></div><div class="sale-incomplete-actions"><select id="saleIncompleteUser${index}" aria-label="Assign incomplete McCoy evidence to user">${options}</select><input id="saleIncompleteReason${index}" maxlength="500" placeholder="Manual correction, assignment, or review reason required" aria-label="Incomplete evidence decision reason"><button id="saleIncompleteApprove${index}" class="primary">APPROVE MANUAL CORRECTION</button><select id="saleIncompleteDisposition${index}" aria-label="Incomplete McCoy evidence review disposition"><option value="">Choose review…</option><option value="unverified">UNVERIFIED</option><option value="not_a_sale">NOT A SALE · REMOVE FROM BANK</option>${row.admin_review_disposition?'<option value="restore">RESTORE</option>':''}</select><button id="saleIncompleteReview${index}" class="assign-btn">Apply Review</button></div><details class="sale-account-details"><summary>McCoy evidence and review history</summary><div class="sale-account-columns"><div class="sale-account-block"><h3>Incomplete McCoy evidence</h3><pre class="sale-account-json">${pretty(row)}</pre></div><div class="sale-account-block"><h3>Evidence correction history</h3><pre class="sale-account-json">${pretty(row.evidence_history||[])}</pre></div><div class="sale-account-block"><h3>Admin review history</h3><pre class="sale-account-json">${pretty(row.review_history||[])}</pre></div></div></details></div>`;
    };
    root.innerHTML=`<p><strong>${Number(bank.count||0)} unassigned provider candidate${Number(bank.count||0)===1?'':'s'}</strong> · <strong>${Number(bank.incomplete_count||0)} incomplete McCoy record${Number(bank.incomplete_count||0)===1?'':'s'}</strong></p>
      ...
      <details open><summary>Incomplete McCoy sale evidence (${incomplete.length})</summary>${incomplete.length?incomplete.map(incompleteMarkup).join(''):'<div class="sale-credit-empty">No incomplete McCoy sale records.</div>'}</details>`;
    const removedRoot=document.getElementById('saleRemovedRows');
    if(removedRoot)removedRoot.innerHTML=`<details><summary>Removed provider evidence (${denied.length})</summary>${denied.length?denied.map((row,index)=>rowMarkup(row,index,true)).join(''):'<div class="sale-credit-empty">No provider evidence has been removed.</div>'}</details>`;
    rows.forEach((row,index)=>bindBankActions(row,index));denied.forEach((row,index)=>bindDeniedBankAction(row,index));incomplete.forEach((row,index)=>bindIncompleteBankActions(row,index));
  }
  ...
  function bindBankActions(row,index){
    ...
    review.onclick=async()=>{const disposition=document.getElementById('saleBankDisposition'+index).value,reason=document.getElementById('saleBankReason'+index).value.trim();if(!disposition){setNotice('Choose UNVERIFIED, NOT A SALE, or RESTORE.');return;}if(!reason){setNotice('Enter an evidence review reason.');return;}review.disabled=true;review.textContent='Applying…';try{const {error}=await sb.rpc('admin_set_provider_evidence_disposition',{p_provider_sale_row_id:row.id,p_disposition:disposition,p_reason:reason});if(error)throw error;const message=disposition==='not_a_sale'?'Provider evidence marked NOT A SALE and removed from the Sales Bank. Its audit record remains under Removed Sales Audit.':disposition==='unverified'?'Provider evidence held as UNVERIFIED and kept in the investigation bank.':'Provider evidence decision restored.';await refreshBankAfterChange(row.id,message);}catch(error){setNotice(error?.message||'Unable to review provider evidence.');review.disabled=false;review.textContent='Apply Review';}};
  }

  function bindDeniedBankAction(row,index){
    const button=document.getElementById('saleBankRestoreDenied'+index);button.onclick=async()=>{const reason=document.getElementById('saleBankReasonDenied'+index).value.trim();if(!reason){setNotice('Enter why this removed provider evidence should be restored.');return;}button.disabled=true;button.textContent='RESTORING…';try{const {error}=await sb.rpc('admin_set_provider_evidence_disposition',{p_provider_sale_row_id:row.id,p_disposition:'restore',p_reason:reason});if(error)throw error;await refreshBankAfterChange(row.id,'Provider evidence restored to the Unassigned Sales Bank.');}catch(error){setNotice(error?.message||'Unable to restore provider evidence.');button.disabled=false;button.textContent='RESTORE TO SALES BANK';}};
  }

  function bindIncompleteBankActions(row,index){
    const approve=document.getElementById('saleIncompleteApprove'+index),review=document.getElementById('saleIncompleteReview'+index),candidateSelect=document.getElementById('saleProviderCandidate'+index),providerApply=document.getElementById('saleProviderApply'+index),providerPreview=document.getElementById('saleProviderPreview'+index),providerCandidates=Array.isArray(row.provider_candidates)?row.provider_candidates:[];
    if(candidateSelect){
      candidateSelect.onchange=()=>{
        const candidate=providerCandidates.find(item=>item.id===candidateSelect.value);providerApply.disabled=!candidate;
        providerPreview.innerHTML=candidate?`<strong>${esc(candidate.provider)} · ${esc(candidate.order_number||candidate.account_number||'No order/account')}</strong><br>Customer: ${esc(candidate.customer_name||'Missing')} · Address: ${esc(candidate.service_address||'Missing')}<br>Product: ${esc(candidate.product_name||'Missing')} · Sale date: ${esc(candidate.sale_date||'Missing')} · Install date: ${esc(candidate.install_date||'Missing')}${candidate.will_archive_provider_duplicate?'<br><span class="sale-credit-missing">A provider-origin duplicate will be removed from rankings and accounting.</span>':''}`:'Choose an ISP export to preview the customer, order, address, product, and dates that can fill blank accounting fields.';
      };
      providerApply.onclick=async()=>{
        const providerSaleRowId=candidateSelect.value,reason=document.getElementById('saleProviderReason'+index).value.trim(),candidate=providerCandidates.find(item=>item.id===providerSaleRowId);
        if(!candidate){setNotice('Choose the matching ISP export row.');return;}if(!reason){setNotice('Enter why this ISP order matches the user-confirmed sale.');return;}
        const duplicateNotice=candidate.will_archive_provider_duplicate?' The provider-origin duplicate will be removed from rankings and accounting.':'';
        if(!window.confirm(`Fill only blank accounting fields on ${row.rep_name||row.rep_email}'s confirmed sale from ISP order ${candidate.order_number||candidate.account_number||candidate.id}?${duplicateNotice}`))return;
        providerApply.disabled=true;providerApply.textContent='APPLYING ISP DATA…';
        try{
          const {data,error}=await sb.rpc('admin_verify_confirmed_sale_from_provider',{p_sale_id:row.id,p_provider_sale_row_id:providerSaleRowId,p_reason:reason});if(error)throw error;
          const filled=Array.isArray(data?.filled_fields)?data.filled_fields:[];
          await refreshBankAfterChange(providerSaleRowId,`ISP export verified for ${data?.sale?.rep_name||data?.sale?.rep_email||row.rep_name||row.rep_email}. Filled ${filled.length?filled.join(', '):'no previously blank fields'}; rankings and accounting now use the user-confirmed sale.${data?.archived_duplicate_sale_id?' The provider-origin duplicate was archived.':''}`,true);
        }catch(error){setNotice(error?.message||'Unable to reconcile the ISP export with this sale.');providerApply.disabled=false;providerApply.textContent='FILL FROM ISP EXPORT & VERIFY';}
      };
    }
    approve.onclick=async()=>{
      const email=document.getElementById('saleIncompleteUser'+index).value,reason=document.getElementById('saleIncompleteReason'+index).value.trim();
      if(!email){setNotice('Choose an active McCoy user for this incomplete sale.');return;}if(!reason){setNotice('Enter why the evidence was corrected and assigned.');return;}
      approve.disabled=true;approve.textContent='APPROVING…';
      try{
        const args={p_sale_id:row.id,p_rep_email:email,p_order_number:document.getElementById('saleIncompleteOrder'+index).value,p_customer_first_name:document.getElementById('saleIncompleteFirst'+index).value,p_customer_last_name:document.getElementById('saleIncompleteLast'+index).value,p_service_address:document.getElementById('saleIncompleteAddress'+index).value,p_install_date:document.getElementById('saleIncompleteInstall'+index).value||null,p_order_date:document.getElementById('saleIncompleteDate'+index).value||null,p_reason:reason};
        const {data,error}=await sb.rpc('admin_resolve_incomplete_sale_evidence',args);if(error)throw error;
        await refreshBankAfterChange(row.id,`Incomplete McCoy evidence corrected, approved, and assigned to ${data.rep_name||data.rep_email}; rankings and pay were updated.`,true);
      }catch(error){setNotice(error?.message||'Unable to approve incomplete McCoy evidence.');approve.disabled=false;approve.textContent='APPROVE MANUAL CORRECTION';}
    };
    review.onclick=async()=>{
      const disposition=document.getElementById('saleIncompleteDisposition'+index).value,reason=document.getElementById('saleIncompleteReason'+index).value.trim();
      if(!disposition){setNotice('Choose UNVERIFIED, NOT A SALE, or RESTORE.');return;}if(!reason){setNotice('Enter an evidence review reason.');return;}
      review.disabled=true;review.textContent='Applying…';
      try{const {error}=await sb.rpc('admin_set_sale_review_disposition',{p_sale_id:row.id,p_disposition:disposition,p_reason:reason});if(error)throw error;const message=disposition==='not_a_sale'?'Incomplete McCoy evidence marked NOT A SALE and removed from the Sales Bank, rankings, and earned-sale accounting.':disposition==='unverified'?'Incomplete McCoy evidence held as UNVERIFIED and kept in the investigation bank.':'Admin review decision restored; the record remains blocked until every required field is complete.';await refreshBankAfterChange(row.id,message,true);}catch(error){setNotice(error?.message||'Unable to review incomplete McCoy evidence.');review.disabled=false;review.textContent='Apply Review';}
    };
  }
