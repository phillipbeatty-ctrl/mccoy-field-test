...
(function(){
  ...
  css.textContent=`
    ...
    @media(max-width:760px){.sale-credit-summary,.sale-credit-actions,.sale-credit-review,.sale-credit-payment,.sale-account-columns,.sale-bank-grid{grid-template-columns:1fr}}
  `;
  ...
  function ensurePanel(){
    ...
    const panel=document.createElement('div');panel.id='saleCreditPanel';panel.innerHTML=`<div class="sale-credit-card">
      ...
      <div class="sale-credit-toolbar"><select id="saleCreditFilter" aria-label="Sale record filter"><option value="verified">VERIFIED sales</option><option value="review_queue">REVIEW QUEUE</option><option value="unverified">UNVERIFIED sales</option><option value="not_a_sale">NOT A SALE</option><option value="all">All sales</option></select><select id="saleCreditSort" aria-label="Sort Sale Credit list"><option value="newest">Sort: Newest</option><option value="isp">Sort: ISP</option><option value="region">Sort: Region</option></select><input id="saleCreditSearch" placeholder="Search any account, customer, address, order, provider, region, or rep data" aria-label="Search all sale account data"><button id="saleCreditRefresh" class="assign-btn">Refresh</button></div>
  ...
  function renderSales(){
    ...
    const filter=document.getElementById('saleCreditFilter')?.value||'verified',sort=document.getElementById('saleCreditSort')?.value||'newest',needle=(document.getElementById('saleCreditSearch')?.value||'').trim().toLowerCase();
  ...
  function saleRow(record,index){
    ...
    return `<article class="sale-credit-row">
      ...
      <div class="sale-credit-actions"><select id="saleCreditUser${index}" aria-label="Ranked or credited McCoy user">${options}</select><input id="saleCreditReason${index}" maxlength="240" placeholder="Credit or approval reason required" aria-label="Credit or approval reason"><button id="saleCreditSave${index}" class="primary">Apply Credit</button><button id="saleCreditApprove${index}" class="${adminApproved?'sale-credit-approved':'primary'}" title="Verify the selected rep for rankings and pay" ${noRep?'disabled':''}>APPROVED</button></div>
  ...
  function bindSaleActions(record,index){
    ...
    approveButton.onclick=async()=>{
      const email=userSelect.value,reason=document.getElementById('saleCreditReason'+index).value.trim();
      if(!email){setNotice('Choose a rep before approving the sale.');return;}if(!reason){setNotice('Enter an approval reason.');return;}
      approveButton.disabled=true;approveButton.textContent='APPROVING…';
      try{const {data,error}=await sb.rpc('admin_approve_sale_credit',{p_sale_id:sale.id,p_rep_email:email,p_reason:reason});if(error)throw error;await refreshAfterChange(sale.id,`Verified Sale approved for ${data.rep_name||data.rep_email}. It is applied to rankings and ${data.competition_eligible===true?'pay':'the cancellation/chargeback accounting path'}.`);}catch(error){setNotice(error?.message||'Unable to approve sale credit.');approveButton.disabled=false;approveButton.textContent='APPROVED';}
    };
