...
(function(){
  ...
  style.textContent='button[id^="saleCreditApprove"]{display:none!important}';
  ...
  function ensure(){
    ...
    panel.innerHTML=`<div style="width:min(1180px,100%);max-height:94vh;overflow:auto;background:#fff;border-radius:16px;padding:18px">
      <div class="card-head"><div><h2>Admin Sale Review</h2><p class="muted small">Choose the authoritative credited user, correct the customer/order details, optionally attach provider evidence, then press APPROVED. The corrections and approval commit together.</p></div><button id="adminSaleReviewCloseTop" class="assign-btn">Close</button></div>
  ...
  async function load(force=false){
    ...
    try{
      ...
      notice.textContent=`Loaded ${records.length} sales. APPROVED saves the edited customer information and approval in one transaction.`;
  ...
  function renderForm(){
    ...
    root.innerHTML=`
      <div style="padding:12px;border:1px solid #dbe4f0;border-radius:12px;background:#f8fafc"><strong>Current credited user:</strong> ${esc(s.rep_name||s.rep_email||'Unknown')} &nbsp; <strong>Provider seller:</strong> ${esc(s.provider_reported_rep_name||s.provider_reported_rep_email||record.provider_account?.seller_name||'Not reported')}<br><span class="muted small">The credited user selected below is authoritative after approval. Provider seller/original processing evidence remains preserved.</span></div>
      ...
      <div style="display:flex;justify-content:flex-end;margin-top:12px"><button id="asrSave" class="primary" style="min-width:180px">APPROVED</button></div>
      <div class="muted small" style="margin-top:8px">APPROVED saves all edited fields shown above, preserves provider evidence, applies the selected credited user, and updates ranking eligibility together. No separate reason is required.</div>`;
    document.getElementById('asrSave').onclick=save;
  }

  function val(id){return document.getElementById(id)?.value??'';}
  async function save(){
    const s=state.selected?.sale;if(!s)return;const rep=val('asrRep');
    if(!rep){document.getElementById('adminSaleReviewNotice').textContent='Choose the authoritative credited user.';return;}
    const corrections={
    ...
    const button=document.getElementById('asrSave');button.disabled=true;button.textContent='APPROVING…';
    try{
      const {data,error}=await sb.rpc('admin_review_sale_transaction',{p_sale_id:s.id,p_credited_rep_email:rep,p_reason:'Admin approved sale review',p_provider_sale_row_id:val('asrProvider')||null,p_corrections:corrections});if(error)throw error;
      const savedId=s.id;
      document.getElementById('adminSaleReviewNotice').textContent=`APPROVED. Customer information saved and sale credited to ${data.rep_name||data.rep_email}.`;
      state.records=[];await load(true);state.selected=state.records.find(r=>r.sale?.id===savedId)||null;renderSaleOptions();renderForm();
      window.dispatchEvent(new CustomEvent('mccoy-sale-credit-changed',{detail:{saleId:savedId}}));window.dispatchEvent(new CustomEvent('mccoy-live-sales-changed',{detail:{saleId:savedId}}));
    }catch(error){document.getElementById('adminSaleReviewNotice').textContent=error?.message||'Approval failed. No changes were committed.';button.disabled=false;button.textContent='APPROVED';}
  }
