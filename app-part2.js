...
document.getElementById('arriveDoorBtn')?.addEventListener('click',()=>{
  ...
  if(state.lastDispositionEndedAt){
    saveTestEvent({eventType:'transition_interval',leadLabel:lead.address,eventTime:arrivedAt,gps,dwellMs:arrivedAt-state.lastDispositionEndedAt,payload:{rawEvent:true,fromDispositionToNextPhysicalKnock:true}});
  }
...
document.querySelectorAll('[data-disp]').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  state.lastDispositionEndedAt=endedAt;
